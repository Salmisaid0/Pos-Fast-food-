...
function testCatalogManagementSnapshotTracksInactiveSeedProducts(): void {
  const snapshot = createCatalogManagementSnapshot();
  const activeProducts = filterActiveProducts(seedProducts, { searchTerm: "shake" });
  const drinksCategory = snapshot.categories.find((item) => item.category.id === "category-drinks");

  assert(
    snapshot.totalProductCount === seedProducts.length,
    "Catalog snapshot should count all products"
  );
  assert(snapshot.inactiveProductCount === 1, "Catalog snapshot should count inactive products");
  assert(
    snapshot.inactiveProducts[0]?.sku === "DRK-999",
    "Catalog snapshot should list inactive seed products"
  );
  assert(activeProducts.length === 0, "Inactive products should stay hidden from cashier search");
  assert(
    drinksCategory?.inactiveProductCount === 1,
    "Catalog snapshot should count inactive category items"
  );
}
...
async function testLocalStoreRuntimeResolverAndJsonFactory(): Promise<void> {
  const nodeTarget = resolveLocalStoreTarget({ POS_LOCAL_STORE_PATH: "/tmp/pos-local-store.json" });
  const tauriTarget = resolveLocalStoreTarget({
    POS_TAURI_LOCAL_STORE_PATH: "/app/data/pos-local-store.json",
  });
  const browserTarget = resolveLocalStoreTarget({ POS_BROWSER_LOCAL_STORE_KEY: "browser-key" });

  if (nodeTarget.driver !== "node-json") {
    throw new Error("Runtime resolver should support node JSON store path");
  }
  assert(
    nodeTarget.filePath === "/tmp/pos-local-store.json",
    "Runtime resolver should keep node JSON store path"
  );
  assert(
    tauriTarget.driver === "tauri-json" &&
      tauriTarget.filePath === "/app/data/pos-local-store.json",
    "Runtime resolver should reserve future Tauri app-data path"
  );
  assert(
    browserTarget.driver === "browser-local-storage" && browserTarget.key === "browser-key",
    "Runtime resolver should support browser localStorage key"
  );
  assert(
    createLocalJsonSaleRepositoriesFromTarget(nodeTarget) instanceof LocalJsonSaleRepositories,
    "Node JSON target should create JSON sale repositories"
  );
}
...
async function testPosOutboxSyncServiceFlushesSuccessfully(): Promise<void> {
  const repositories = new InMemoryLocalSaleRepositories();
  const sale = await finalizeCartCashSale({
    cart: addProductToCart(createEmptyCart(), seedProducts[0]!),
    receivedDZD: 500,
  ...
  const snapshot = await flushOutboxOnce({
    repositories,
    api: {
  ...
  assert(snapshot.status === "SYNCED", "POS sync service should mark successful flush as synced");
  assert(
    snapshot.syncedCount === sale.syncEvents.length,
    "POS sync service should report all synced events"
  );
  assert(snapshot.pendingCount === 0, "Successful POS sync should leave no pending events");
  assert(pushed.size === sale.syncEvents.length, "POS sync service should push every event once");
  assert(
    entries.every((entry) => entry.status === "SYNCED"),
    "POS sync service should mark every outbox entry as synced"
  );
}

async function testPosOutboxSyncServiceRetriesFailures(): Promise<void> {
  const repositories = new InMemoryLocalSaleRepositories();
  const sale = await finalizeCartCashSale({
    cart: addProductToCart(createEmptyCart(), seedProducts[1]!),
    receivedDZD: 1000,
    localSequence: 5,
    finalizedAt: now,
    repositories,
  ...
  const api: RemoteSyncApi = {
    async pushEvent() {
      if (apiIsDown) throw new Error("network unavailable");
    },
  };

  const failedSnapshot = await flushOutboxOnce({ repositories, api, now: () => new Date(now) });
  const failedEntries = await repositories.outbox.listEntries();
  apiIsDown = false;
  const retriedSnapshot = await flushOutboxOnce({ repositories, api, now: () => new Date(now) });
  const retriedEntries = await repositories.outbox.listEntries();

  assert(failedSnapshot.status === "FAILED", "POS sync should report failed network flushes");
  assert(
    failedSnapshot.failedAttemptCount === sale.syncEvents.length,
    "Failed POS sync should report every failed attempt"
  );
  assert(
    failedEntries.every((entry) => entry.status === "FAILED" && entry.event.attemptCount === 1),
    "Failed POS sync should keep failed events retryable with attempt counts"
  );
  assert(retriedSnapshot.status === "SYNCED", "Retry should sync previously failed events");
  assert(
    retriedEntries.every((entry) => entry.status === "SYNCED"),
    "Retry should mark all previously failed events as synced"
  );
}

async function testPosOutboxSyncLoopCanFlushOnDemand(): Promise<void> {
  const repositories = new InMemoryLocalSaleRepositories();
  await finalizeCartCashSale({
    cart: addProductToCart(createEmptyCart(), seedProducts[2]!),
    receivedDZD: 900,
    localSequence: 6,
    finalizedAt: now,
    repositories,
  ...
  const controller = startOutboxSyncLoop({
    repositories,
    api: {
      async pushEvent() {
        return;
  ...
  const snapshot = await controller.flushNow();
  controller.stop();

  assert(snapshot.status === "SYNCED", "Manual POS sync loop flush should sync outbox events");
  assert(
    observedStatuses[0] === "SYNCING" && observedStatuses.at(-1) === "SYNCED",
    "POS sync loop should emit syncing and synced states"
  );
}

async function testHttpRemoteSyncApiPostsJsonEvents(): Promise<void> {
  const repositories = new InMemoryLocalSaleRepositories();
  const sale = await finalizeCartCashSale({
    cart: addProductToCart(createEmptyCart(), seedProducts[3]!),
    receivedDZD: 1000,
    localSequence: 7,
    finalizedAt: now,
    repositories,
  ...
  const api = new HttpRemoteSyncApi({
    endpoint: "http://localhost:3000/sync/events",
    async fetchImpl(input, init) {
      assert(input === "http://localhost:3000/sync/events", "HTTP sync should post to endpoint");
      assert(init.method === "POST", "HTTP sync should use POST");
      assert(
        init.headers["content-type"] === "application/json",
        "HTTP sync should send JSON content type"
      );
  ...
  await api.pushEvent(sale.syncEvents[0]!);
  assert(postedBodies.length === 1, "HTTP sync should send one request per event");
  assert(
    (JSON.parse(postedBodies[0]!) as SyncEvent).id === sale.syncEvents[0]!.id,
    "HTTP sync should serialize the sync event body"
  );
}

async function testLocalPrintStatusSnapshotTracksQueuedFailedAndSentRequests(): Promise<void> {
  const repositories = new InMemoryLocalSaleRepositories();
  const sale = await finalizeCartCashSale({
    cart: addProductToCart(createEmptyCart(), seedProducts[0]!),
    receivedDZD: 500,
  ...
  if (!sale.printerJob) throw new Error("Sale should request a receipt print job");

  const queuedSnapshot = await loadLocalPrintStatusSnapshot(repositories, 5);
  assert(queuedSnapshot.queuedCount === 1, "Print status should count queued print requests");
  assert(
    queuedSnapshot.jobs[0]?.printerJobId === sale.printerJob.id,
    "Print status should include the requested print job"
  );
  assert(
    formatLocalPrintJobStatus(queuedSnapshot.jobs[0]!.status) === "Queued for server print",
    "Print status should format queued print jobs"
  );

  await repositories.outbox.markFailed(
    `${sale.printerJob.id}:print-job-requested`,
    new Error("api offline"),
    now
  );
  const failedSnapshot = await loadLocalPrintStatusSnapshot(repositories, 5);
  assert(failedSnapshot.failedCount === 1, "Print status should count failed print requests");
  assert(
    failedSnapshot.jobs[0]?.lastError === "api offline",
    "Print status should expose print sync errors"
  );

  await repositories.outbox.markSynced(`${sale.printerJob.id}:print-job-requested`, now);
  const sentSnapshot = await loadLocalPrintStatusSnapshot(repositories, 5);
  assert(sentSnapshot.sentCount === 1, "Print status should count print requests sent to server");
  assert(
    sentSnapshot.jobs[0]?.status === "SENT_TO_SERVER",
    "Print status should mark synced print requests as sent"
  );
}

async function testSyncRecoverySnapshotListsRetryableEvents(): Promise<void> {
  const repositories = new InMemoryLocalSaleRepositories();
  const sale = await finalizeCartCashSale({
    cart: addProductToCart(createEmptyCart(), seedProducts[0]!),
    receivedDZD: 500,
  ...
  const failedEvent = sale.syncEvents[0]!;

  await repositories.outbox.markFailed(failedEvent.id, new Error("network unavailable"), now);

  const snapshot = await loadSyncRecoverySnapshot(repositories, 10);
  const failedRecoveryEvent = snapshot.failedEvents[0]!;

  assert(snapshot.hasRecoveryWork, "Sync recovery should report retryable work");
  assert(snapshot.failedEvents.length === 1, "Sync recovery should list failed events");
  assert(
    snapshot.pendingEvents.length === sale.syncEvents.length - 1,
    "Sync recovery should list pending events"
  );
  assert(
    snapshot.retryableEvents.length === sale.syncEvents.length,
    "Sync recovery should list all retryable events"
  );
  assert(
    failedRecoveryEvent.eventId === failedEvent.id,
    "Sync recovery should expose failed event id"
  );
  assert(
    failedRecoveryEvent.lastError === "network unavailable",
    "Sync recovery should expose last error"
  );
  assert(failedRecoveryEvent.attemptCount === 1, "Sync recovery should expose retry attempts");
  assert(
    formatRecoverableSyncEvent(failedRecoveryEvent).includes("1 attempt"),
    "Sync recovery label should include retry attempts"
  );
}

async function testLocalSalesBackupValidationRejectsCorruptRecoveryFiles(): Promise<void> {
  const repositories = new InMemoryLocalSaleRepositories();
  await finalizeCartCashSale({
    cart: addProductToCart(createEmptyCart(), seedProducts[0]!),
    receivedDZD: 500,
  ...
  const backup = await createLocalSalesBackup(repositories, { exportedAt: now });
  const validParsedBackup = parseLocalSalesBackupPayload(serializeLocalSalesBackup(backup));
  const invalidJsonBackup = parseLocalSalesBackupPayload("not-json");
  const mismatchedCountBackup = validateLocalSalesBackup({
    ...backup,
    counts: {
      ...backup.counts,
      orders: backup.counts.orders + 1,
    },
  });

  assert(validParsedBackup.ok, "Backup validation should accept exported backup JSON");
  assert(!invalidJsonBackup.ok, "Backup validation should reject invalid JSON");
  assert(!mismatchedCountBackup.ok, "Backup validation should reject mismatched counts");
  assert(
...
async function testLocalSalesBackupContainsDurableSaleData(): Promise<void> {
  const repositories = new InMemoryLocalSaleRepositories();
  const sale = await finalizeCartCashSale({
    cart: addProductToCart(createEmptyCart(), seedProducts[0]!),
    receivedDZD: 500,
  ...
  const backup = await createLocalSalesBackup(repositories, { exportedAt: now });
  const serialized = serializeLocalSalesBackup(backup);
  const filename = createLocalSalesBackupFilename(backup.exportedAt);

  assert(backup.schemaVersion === 1, "Local backup should include schema version");
  assert(backup.counts.orders === 1, "Local backup should count orders");
  assert(backup.counts.payments === 1, "Local backup should count payments");
  assert(backup.counts.receipts === 1, "Local backup should count receipts");
  assert(
    backup.counts.outboxEntries === sale.syncEvents.length,
    "Local backup should count outbox entries"
  );
  assert(
    backup.orders[0]?.id === sale.order.id,
    "Local backup should include finalized local order"
  );
  assert(backup.payments[0]?.id === sale.payment.id, "Local backup should include cash payment");
  assert(backup.receipts[0]?.id === sale.receipt.id, "Local backup should include receipt");
  assert(
    backup.outboxEntries[0]?.event.id === sale.syncEvents[0]?.id,
    "Local backup should include outbox events"
  );
  assert(
    serialized.includes(sale.receipt.receiptNumber),
    "Serialized backup should include receipt"
  );
  assert(filename.startsWith("pos-local-sales-backup-"), "Backup filename should be prefixed");
}
...
async function main(): Promise<void> {
  ...
  testCatalogManagementSnapshotTracksInactiveSeedProducts();
  ...
  await testLocalStoreRuntimeResolverAndJsonFactory();
  ...
  await testPosOutboxSyncServiceRetriesFailures();
  await testPosOutboxSyncLoopCanFlushOnDemand();
  await testHttpRemoteSyncApiPostsJsonEvents();
