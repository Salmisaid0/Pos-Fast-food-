...
export interface SyncIngestionSideEffects {
  enqueuePrinterJob(printerJob: PrinterJob): Promise<void>;
}
...
export async function ingestSyncEvent(
  ...
  await persistEventEffect(event, repository, sideEffects);
...
export async function ingestSyncEvents(
  ...
  for (const event of events) {
    results.push(await ingestSyncEvent(event, repository, sideEffects));
  }
...
async function persistEventEffect(
  ...
  switch (event.type) {
    ...
    case "PRINT_JOB_REQUESTED":
      ...
      await sideEffects?.enqueuePrinterJob(event.payload.printerJob);
      return;
  }
}
