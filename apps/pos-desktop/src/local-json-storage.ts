...
import type { LocalStoreTarget } from "./local-store-runtime";
import type {
...
export function createLocalJsonSaleRepositoriesFromTarget(
  target: Extract<LocalStoreTarget, { driver: "node-json" | "tauri-json" }>
): LocalJsonSaleRepositories {
  return new LocalJsonSaleRepositories(target.filePath);
}
